"""Lightweight JSON-backed internationalisation helpers for the CSA app."""
...
def get_locale() -> str:
    ...
    manager: TranslationManager | None = current_app.extensions.get("i18n") if current_app else None
    if not manager:
        return "en"
...
def _fallback_translator(locale: str) -> TranslationManager:
    dummy_path = (Path(__file__).resolve().parent / "translations").resolve()
...
class LazyString:
    """Callable string that evaluates a translation when rendered."""
...
def lazy_gettext(key: str, **variables: Any) -> LazyString:
    return LazyString(gettext, key, **variables)
