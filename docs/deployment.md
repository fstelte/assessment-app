...
- The components CSV ends with a `Component Tier` column (the component's own tier as `TIER n > name`, blank when it inherits the BIA tier). CSV import resolves tiers by level, so it survives databases where tier ids differ; an unknown tier imports as "inherit" and a warning is shown. SQL exports carry component and BIA `tier_id` values instead, which SQL import keeps only when a tier with that id exists. Restoring an SQL export into a database whose tier ids differ can therefore assign a different tier, so prefer the CSV route across environments or check the tiers afterwards.
